...
float MyGetReward(void)
{
    ...
    const double penalty_scale = 1.0; 
    const double strict_order_penalty = 10.0;  
    const double dynamic_penalty_coeff = 20.0; 
    ...
    for(const auto &pair : flowAllocations){
        ...
        for (size_t i = 1; i < completionTimes.size(); ++i) {
            if (completionTimes[i] <= completionTimes[i-1] &&  // 改为<=确保严格递增
                completionTimes[i] != INFINITY && 
                completionTimes[i-1] != INFINITY) {
                // 固定惩罚（确保违反时代价高昂）
                flowPenalty -= strict_order_penalty;
                // 动态惩罚（时间差越大惩罚越重）
                double gapRatio = (completionTimes[i-1] - completionTimes[i]) / completionTimes[i-1];
                flowPenalty -= gapRatio * dynamic_penalty_coeff;  
            }
        }
        // for (size_t i = 1; i < completionTimes.size(); ++i) {
        //     // 跳过无穷大的完成时间，不参与顺序检查
        ...
        cout << "Flow " << flowIdx << " Reward: " << flowReward 
            << " (Time: " << -alpha * maxCompletionTime * 100 << ", Util: " << beta * avgUtilization * 100 
            << ", Penalty: " << penalty_scale * flowPenalty << ")" << endl;
