...
float MyGetReward(void)
{
    ...
    const double penalty_scale = 0.1; 
    ...
    for(const auto &pair : flowAllocations){
        ...
        for (size_t i = 0; i < maxPathCount; i++){
            ...
            if(i < paths.size() && i < allocation.size()){
                ...
                // 时间单位转换为 s
                double path_time = transmissionTime + p.delay * 1e-9;
            ...
            if (path_time == std::numeric_limits<double>::infinity()) {
                flowPenalty -= 1.0;
            }
        ...
        if (!hasValidPath) {
            maxCompletionTime = std::numeric_limits<double>::infinity();
            flowPenalty -= 5.0;
        }
        ...
        double flowReward = -alpha * maxCompletionTime + beta * avgUtilization + penalty_scale * flowPenalty;
        totalReward += flowReward;
        cout << "Flow " << flowIdx << " Reward: " << flowReward 
                << " (Time: " << maxCompletionTime << ", Util: " << avgUtilization 
                << ", Penalty: " << flowPenalty << ")" << endl;
