...
observation - 每个节点队列长度
actions - 为每个节点设置通道接入概率，将 CWmin 和 CWMax 设置为相同的值 例如：均匀退避uniform backoff 数据包冲突的情况下保持不变，频道接入概率
reward - 目的节点接收到的数据包数量
gameover - 设置仿真结束
 */
...
收集状态：每个节点的队列长度存入 box 类型空间 GetQueue 进一步 GetNPackets
*/
...
bool MyExecuteActions(Ptr<OpenGymDataContainer> action)
{
  ...
  std::cout << "MyExecuteActions函数执行" << std::endl;
