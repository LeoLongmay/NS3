...
size_t sample_flow_size = 10000; // 采样流的数量
...
struct FlowInput
{
    ...
    // 开始时间
    double start_time;
    // 源节点、目的节点、优先级组（不确定，猜测，目前来看用不上），最大包数量，端口
 ...
 * 名称叫动作执行函数，实际上应该叫动作向量处理函数，函数的功能是将动作向量归一化后处理为概率向量
 */
...
float MyGetReward(void)
{
    ...
    for (size_t i = 0; i < paths.size(); i++)
    {
        ...
        for (size_t j = 1; j < p.path.size(); j++)
        {
            ...
            {
                cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
                cout << "node " << node1 << " to " << "node " << node2 << endl;
                cout << "nextAvaliableTime: " << nextAvailableTime[node1][node2] << endl;
                cout << "nowTime: " << Simulator::Now() << endl;
                cout << "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" << endl;
                failedFlowCount++;
                return -0.1;
            }
    ...
    cout << "******************************************************************" << endl;
    std::cout << "CurrFlowId: " << curflow.idx << endl;
    std::cout << "CurrFlowStartTime: " << curflow.start_time << endl;
    cout << "paths: " << paths.size() << endl;
    cout << "srcNode: " << srcNode->GetId() << " dstNode: " << dstNode->GetId() << endl;
    ...
    for (size_t i = 0; i < paths.size(); i++)
    {
        ...
        cout << "i: " << i << " currAction: " << currentAction[i] << " transTime: " << transmissionTime << endl;
    ...
    // std::cout << "CurrFlowTrasmissionTime: " << totalTransmissionTime << endl;
 ...
 * @return 采样后的流量集合
 */
vector<FlowInput> SampleFlows(size_t sampleSize = sample_flow_size) 
{
    // 采样的流数量不超过最大流数量
    sampleSize = sampleSize > flow_num ? flow_num : sampleSize; 
...
{
    uint32_t simSeed = 1;
    uint32_t openGymPort = 5010;
    double simulationTime = 1;
    // 模拟时钟的步长为1纳秒
    // double envStepTime = 0.000000001;
    // 将步长改为500纳秒
    double envStepTime = 500 * 0.000000001;
    ...
    fflush(stdout);
    cout << "Run start at: " << Simulator::Now() << endl;
