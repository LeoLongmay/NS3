...
size_t sample_flow_size = 500000; // 采样流的数量
...
// 流日志结构体，用于查询两个节点之间的流是否具有周期性，和FlowInput结构体很像
struct FlowLog {
    // 流id
    uint32_t idx;
    // 开始时间
...
struct OpticalPathInfo {
    ...
    uint32_t hops;
    uint32_t pathCount;
    OpticalPathInfo(): delay(0), txDelay(0), bw(0xfffffffffffffffflu), hops(0), pathCount(0) {}
};
