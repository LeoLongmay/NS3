...
class FcnHeader : public Header 
{
public:
  FcnHeader (const uint16_t fid, uint8_t qIndex, uint8_t ecnbits, uint16_t qfb, uint16_t total, uint64_t timestamp, uint16_t flowCount);
  //FcnHeader (const uint16_t fid, uint8_t qIndex, uint8_t qfb);
  FcnHeader ();
  virtual ~FcnHeader ();
