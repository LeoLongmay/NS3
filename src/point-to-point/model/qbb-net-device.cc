...
RdmaEgressQueue::RdmaEgressQueue() {
    ...
    m_ackQ = CreateObject<DropTailQueue<Packet>>(); // 修改代码,指定模板参数
    // 修改代码：MaxBytes ————>MaxSize
    m_ackQ->SetAttribute("MaxSize", QueueSizeValue(QueueSize(QueueSizeUnit::PACKETS, 10))); 
    // 如何添加属性   ？ 究竟添加哪个属性是正确的呢？ MaxBytes MaxPackets MaxSize
    // m_ackQ->SetAttribute("MaxPackets",
    ...
    static TypeId tid =
        TypeId("ns3::QbbNetDevice")
            ...
            .AddAttribute("TxBeQueue", "A queue to use as the transmit queue in the device.",
                          ...
                          MakePointerChecker<Queue<Packet>>())  // 修正模板参数
...
void QbbNetDevice::DoDispose() {
    ...
    PointToPointNetDevice::Dispose();  // 调用基类的公共 Dispose 方法
}
...
void QbbNetDevice::DequeueAndTransmit(void) {
    std::cout << "QBBNetDevice_DequeueAndTransmit" << m_queue << std::endl;
    NS_LOG_FUNCTION(this);
...
bool QbbNetDevice::SwitchSend(uint32_t qIndex, Ptr<Packet> packet, CustomHeader &ch) {
   std::cout << "QBBNetDevice_SwitchSend" << m_queue <<std::endl;
    m_macTxTrace(packet);
...
uint32_t QbbNetDevice::SendPfc(uint32_t qIndex, uint32_t type) {
    std::cout << "QBBNetDevice_SendPfc" << m_queue << std::endl;
...
void QbbNetDevice::SetQueue(Ptr<BEgressQueue> q) {
    ...
    m_queue = q;
    // std::cout << "QBBNetDevice_SetQueue\t" << m_queue << std::endl;
}

Ptr<BEgressQueue> QbbNetDevice::GetQueue() { 
    // std::cout << "QBBNetDevice_GetQueue\t" << m_queue << std::endl;
    return m_queue; 
}
...
void QbbNetDevice::TakeDown() {
    // std::cout << "QBBNetDevice_TakeDown" << m_queue << std::endl;
