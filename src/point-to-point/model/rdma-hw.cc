...
int RdmaHw::ReceiverCheckSeq(uint32_t seq, Ptr<RdmaRxQueuePair> q, uint32_t size, bool &cnp) {
    ...
    if (seq == expected || (seq < expected && seq + size >= expected)) {
        if (m_irn) {
            ...
            if (static_cast<uint32_t>(q->m_milestone_rx) < seq + size) q->m_milestone_rx = seq + size;
        ...
        q->ReceiverNextExpectedSeq += size - (expected - seq);
        // 修改代码:修正不同类型的整数比较
        if (static_cast<uint32_t>(q->ReceiverNextExpectedSeq) >= static_cast<uint32_t>(q->m_milestone_rx)) {
            q->m_milestone_rx +=
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp) {
    ...
    // 添加代码：使用 packet_pos 变量，例如日志记录
    NS_LOG_INFO("Packet position: " << static_cast<uint32_t>(packet_pos));
...
#if 1
    // 将 CalculateTxTime 替换为 CalculateBytesTxTime
    Time sendingTime = Seconds(qp->m_rate.CalculateBytesTxTime(qp->lastPktSize));
    Time new_sendintTime = Seconds(new_rate.CalculateBytesTxTime(qp->lastPktSize));
...
void RdmaHw::CheckRateDecreaseMlx(Ptr<RdmaQueuePair> q) {
    ...
    if (q->mlx.m_decrease_cnp_arrived) {
        ...
        // 类型转换
        q->m_rate = std::max(m_minRate, DataRate(q->m_rate.GetBitRate() * (1 - q->mlx.m_alpha / 2)));
...
void RdmaHw::UpdateRateHp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, bool fast_react) {
    ...
    // bool print = !fast_react || true;注释掉
    if (qp->hp.m_lastUpdateSeq == 0) {  // first RTT
        ...
        if (ih.nhop <= IntHeader::maxHop) {
            ...
            DataRate new_rate;
            int32_t new_incStage = 0;  // 初始化 new_incStage 变量
            ...
            if (!m_multipleRate) {
                ...
                if (updated_any) {
                    ...
                    if (max_c >= 1 || qp->hp.m_incStage >= m_miThresh) {
                        // 修改代码:类型转换
                        new_rate = DataRate(qp->hp.m_curRate.GetBitRate() / max_c + m_rai.GetBitRate());
                        // new_rate = qp->hp.m_curRate / max_c + m_rai;
                        new_incStage = 0;
                    } else {
                        // new_rate = qp->hp.m_curRate + m_rai;
                        // 修改代码:类型转换
                        new_rate = DataRate(qp->hp.m_curRate.GetBitRate() + m_rai.GetBitRate());
                ...
                for (uint32_t i = 0; i < ih.nhop; i++) {
                    if (updated[i]) {
                        ...
                        if (c >= 1 || qp->hp.hopState[i].incStage >= m_miThresh) {
                            ...
                            // 修改代码:类型转换
                            new_rate_per_hop[i] = DataRate(qp->hp.hopState[i].Rc.GetBitRate() / c + m_rai.GetBitRate());
                            new_incStage_per_hop[i] = 0;
                        } else {
                            // new_rate_per_hop[i] = qp->hp.hopState[i].Rc + m_rai;
                            // 修改代码:类型转换
                            new_rate_per_hop[i] = DataRate(qp->hp.hopState[i].Rc.GetBitRate() + m_rai.GetBitRate());
    ...
    if (qp->tmly.m_lastUpdateSeq != 0) {  // not first RTT
        ...
        if (inc) {
            if (qp->tmly.m_incStage < 5) {
                ...
                // 修改代码:类型转换
                qp->m_rate = DataRate(qp->tmly.m_curRate.GetBitRate() + m_rai.GetBitRate());

            } else {
                // qp->m_rate = qp->tmly.m_curRate + m_rhai;
                // 修改代码:类型转换
                qp->m_rate = DataRate(qp->tmly.m_curRate.GetBitRate() + m_rhai.GetBitRate());
            ...
            // 修改代码
            qp->m_rate = std::max(m_minRate, DataRate(qp->tmly.m_curRate.GetBitRate() * c));
...
void RdmaHw::HandleAckDctcp(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch) {
    ...
    if (cnp && qp->dctcp.m_caState == 0) {
        ...
        // 修改代码
        qp->m_rate = DataRate(std::max<uint64_t>(m_minRate.GetBitRate(), static_cast<uint64_t>(qp->m_rate.GetBitRate() * (1 - qp->dctcp.m_alpha / 2))));
#if PRINT_LOG
    ...
    if (qp->dctcp.m_caState == 0 && new_batch)
        ...
        // 修改代码
        qp->m_rate = DataRate(std::min(qp->m_max_rate.GetBitRate(), qp->m_rate.GetBitRate() + m_dctcp_rai.GetBitRate()));
