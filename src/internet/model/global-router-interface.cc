...
GlobalRouter::ProcessPointToPointLink (Ptr<NetDevice> ndLocal, GlobalRoutingLSA *pLSA)
{
  ...
  //
  // 添加夹代码调试
  // std::cout << "ndLocal\t" << ndLocal << "\tch\t" << ch << "\tGetAdjacent (ndLocal, ch)\t" << GetAdjacent (ndLocal, ch) << std::endl;
  Ptr<NetDevice> ndRemote = GetAdjacent (ndLocal, ch);
  // std::cout << "ndRemote\t" << ndRemote << std::endl;
...
//
  // std::cout << "ch类型\t" << ch << "\t&\t" << ch->GetTypeId().GetName() << std::endl;
  // 动态转换 将 ch 转换为了 Qbbch （QbbChann类型）
  Ptr<QbbChannel> qbbCh = DynamicCast<QbbChannel>(ch);
  Ptr<NetDevice> nd3 = qbbCh->GetDevice (0);
  Ptr<NetDevice> nd1 = qbbCh->GetDevice (0);
  Ptr<NetDevice> nd2 = qbbCh->GetDevice (1);
//
