  ...
  do
    {
      ...
      for (uint32_t i = 0; i < n; i++)
        {
          // std::cout << "Initialize: i = " << i << std::endl;
          Object *current = m_aggregates->buffer[i];
          // std::cout << "Initialize: Object type = " << typeid(*current).name() << std::endl;
          if (!current->m_initialized)
            {
              // std::cout << "Initialize: current->m_initialized = false, calling DoStart()" << std::endl;
              // current->DoStart ();  // 修改代码
              // current->Object::DoStart();
              // 确保无论什么类型的current 调用的都是 Object类的函数，而不是子类的函数
              current->Object::DoInitialize();
      ...
      for (uint32_t i = 0; i < n; i++)
        {
          ...
          if (!current->m_disposed)
            {
              // std::cout << "Initialize: current->m_disposed = false, calling DoDispose()" << std::endl;
              current->Object::DoDispose ();
...
Object::DoDelete (void)
{
  ...
  NS_LOG_FUNCTION (this);
  // std::cout << "m_aggregates\t" << m_aggregates << "\tm_aggregates->n\t" << m_aggregates->n << std::endl;
