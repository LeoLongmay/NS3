...
Note: `run-overhead-buffer.sh` *appends* its table to `overhead.md` each run. To
regenerate cleanly, remove the previously appended section (everything from the
`<!-- generated by run-overhead-buffer.sh ... -->` comment onward) before re-running.
