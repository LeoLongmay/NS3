...
std::unordered_map<uint32_t, uint16_t> tcpSrcPortNumder;
std::unordered_map<uint32_t, uint16_t> tcpDstPortNumder;
...
uint32_t flow_num;
std::vector<FlowInput> flow_inputs;
...
bool ParseTopologyHeader(uint32_t& nodeNum, uint32_t& switchNum, uint32_t& torNum, uint32_t& linkNum);
bool ParseFlowLine(const std::string& line, FlowInput& flow);
void LoadFlowInputs() {
	flow_inputs.clear();
	flow_inputs.reserve(flow_num);
	std::string line;
	while (std::getline(flowf, line)) {
		...
		FlowInput flow = {0};
		flow.idx = flow_inputs.size();
		if (!ParseFlowLine(line, flow)) {
			NS_FATAL_ERROR("Invalid flow line: " << line);
		}
		NS_ASSERT(n.Get(flow.src)->GetNodeType() == 0 && n.Get(flow.dst)->GetNodeType() == 0);
		flow_inputs.push_back(flow);
	}
	NS_ABORT_MSG_IF(flow_inputs.size() != flow_num,
	                "FLOW_FILE count mismatch: expected " << flow_num << " flows, loaded "
	                                                      << flow_inputs.size());
}

void InstallConfiguredFlows(double stopTime) {
	for (const auto& flow : flow_inputs) {
		uint16_t sport = portNumder[flow.src][flow.dst]++;
		uint16_t dport = flow.dport;
		if (transport_mode == TRANSPORT_MODE_TCP_BBR) {
			sport = tcpSrcPortNumder[flow.src]++;
			dport = tcpDstPortNumder[flow.dst]++;
			InstallTcpFlow(flow.src, flow.dst, sport, dport, flow.maxPacketCount, flow.start_time, stopTime);
		} else {
			RdmaClientHelper clientHelper(flow.pg, serverAddress[flow.src], serverAddress[flow.dst], sport, dport, flow.maxPacketCount, has_win ? (global_t == 1 ? maxBdp : pairBdp[n.Get(flow.src)][n.Get(flow.dst)]) : 0, global_t == 1 ? maxRtt : pairRtt[flow.src][flow.dst], pairBw[flow.src][flow.dst], Simulator::GetMaximumSimulationTime());
			ApplicationContainer appCon = clientHelper.Install(n.Get(flow.src));
			appCon.Start(Seconds(flow.start_time));
		}
	}
	flowf.close();
}
...
void qp_finish(FILE* fout, Ptr<RdmaQueuePair> q) {
	...
	fprintf(fout, "%u %u %u %u %lu %lu %lu %lu\n",
			sid, did, q->sport, q->dport, q->m_size, q->startTime.GetTimeStep(), fct, standalone_fct);
	fflush(fout);
...
void tcp_flow_finish(FILE* fout, uint32_t sid, uint32_t did, uint16_t sport, uint16_t dport, double totalSize, double start, bool, uint32_t) {
	...
	fprintf(fout, "%u %u %u %u %lu %lu %lu %lu\n",
			sid, did, sport, dport, flowSize, startNs, fct, standaloneFct);
	fflush(fout);
}
...
bool ParseFlowLine(const std::string& line, FlowInput& flow) {
	...
	if (values.size() == 5) {
		flow.src = static_cast<uint64_t>(values[0]);
		flow.dst = static_cast<uint64_t>(values[1]);
		flow.pg = static_cast<uint64_t>(values[2]);
		flow.dport = DestportNumder[flow.src][flow.dst];
		flow.maxPacketCount = static_cast<uint64_t>(values[3]);
		flow.start_time = values[4];
		return true;
	}
	if (values.size() == 6) {
		flow.src = static_cast<uint64_t>(values[0]);
		flow.dst = static_cast<uint64_t>(values[1]);
		flow.pg = static_cast<uint64_t>(values[2]);
		flow.dport = static_cast<uint64_t>(values[3]);
		flow.maxPacketCount = static_cast<uint64_t>(values[4]);
		flow.start_time = values[5];
		return true;
	}
	return false;
}
...
void InstallTcpFlow(uint32_t srcId, uint32_t dstId, uint16_t sport, uint16_t dport, uint64_t flowSize, double startTime, double stopTime) {
	...
	sender->SetAttribute("sendAt", TimeValue(Seconds(startTime)));
...
int main(int argc, char *argv[])
{
	...
	std::string cdfFileName = "traffic_gen/tempcdf.txt";

	unsigned randomSeed = random_seed;
	...
	// LPCC tuning CLI args (silently accepted; applied via Config::SetDefault before object creation)
	uint32_t lpccEpsilonArg = 0;
	uint32_t lpccFcnpMinIntervalUsArg = 0;
	uint32_t lpccPerFlowFcnpCooldownUsArg = 0;
	uint32_t lpccFcnpTopKArg = 0;
	uint32_t lpccFcnpTopKHighArg = 0;
	uint32_t lpccFcnpKHighThreshBytesArg = 0;
	uint32_t lpccThetaUsArg = 0;
	uint32_t lpccIncreaseIntervalUsArg = 0;
	double   lpccIncreaseFactorArg = 0.0;
	double   lpccWrArg = 0.0;
	double   lpccKrArg = 0.0;
	double   lpccQueueTargetRatioArg = 0.0;
	double   lpccDropCapHighArg = 0.0;
	uint32_t lpccAiSuppressMultiplierArg = 0;
	cmd.AddValue("lpccEpsilon", "LPCC epsilon (<=0 keep default)", lpccEpsilonArg);
	cmd.AddValue("lpccFcnpMinIntervalUs", "LPCC switch FCNP min interval us", lpccFcnpMinIntervalUsArg);
	cmd.AddValue("lpccPerFlowFcnpCooldownUs", "LPCC per-flow FCNP cooldown us", lpccPerFlowFcnpCooldownUsArg);
	cmd.AddValue("lpccFcnpTopK", "LPCC FCNP top-K", lpccFcnpTopKArg);
	cmd.AddValue("lpccFcnpTopKHigh", "LPCC FCNP high top-K", lpccFcnpTopKHighArg);
	cmd.AddValue("lpccFcnpKHighThreshBytes", "LPCC FCNP K-high threshold bytes", lpccFcnpKHighThreshBytesArg);
	cmd.AddValue("lpccThetaUs", "LPCC theta us", lpccThetaUsArg);
	cmd.AddValue("lpccIncreaseIntervalUs", "LPCC AI interval us", lpccIncreaseIntervalUsArg);
	cmd.AddValue("lpccIncreaseFactor", "LPCC AI factor beta", lpccIncreaseFactorArg);
	cmd.AddValue("lpccWr", "LPCC FCNP decrease cap wr", lpccWrArg);
	cmd.AddValue("lpccKr", "LPCC FCNP decrease cap kr", lpccKrArg);
	cmd.AddValue("lpccQueueTargetRatio", "LPCC queue target ratio", lpccQueueTargetRatioArg);
	cmd.AddValue("lpccDropCapHigh", "LPCC drop cap high", lpccDropCapHighArg);
	cmd.AddValue("lpccAiSuppressMultiplier", "LPCC AI suppress multiplier", lpccAiSuppressMultiplierArg);

	cmd.Parse (argc, argv);

	// Apply LPCC overrides (only if CLI value > 0, otherwise keep AddAttribute defaults)
	if (lpccEpsilonArg > 0)               Config::SetDefault("ns3::RdmaHw::LpccEpsilon",               UintegerValue(lpccEpsilonArg));
	if (lpccFcnpMinIntervalUsArg > 0)     Config::SetDefault("ns3::SwitchNode::FcnpMinIntervalUs",     UintegerValue(lpccFcnpMinIntervalUsArg));
	if (lpccPerFlowFcnpCooldownUsArg > 0) Config::SetDefault("ns3::SwitchNode::LpccPerFlowFcnpCooldownUs", UintegerValue(lpccPerFlowFcnpCooldownUsArg));
	if (lpccFcnpTopKArg > 0)              Config::SetDefault("ns3::SwitchNode::LpccFcnpTopK",          UintegerValue(lpccFcnpTopKArg));
	if (lpccFcnpTopKHighArg > 0)          Config::SetDefault("ns3::SwitchNode::LpccFcnpTopKHigh",      UintegerValue(lpccFcnpTopKHighArg));
	if (lpccFcnpKHighThreshBytesArg > 0)  Config::SetDefault("ns3::SwitchNode::LpccFcnpKHighThreshBytes", UintegerValue(lpccFcnpKHighThreshBytesArg));
	if (lpccThetaUsArg > 0)               Config::SetDefault("ns3::RdmaHw::LpccTheta",                 DoubleValue((double)lpccThetaUsArg));
	if (lpccIncreaseIntervalUsArg > 0)    Config::SetDefault("ns3::RdmaHw::LpccIncreaseInterval",      UintegerValue(lpccIncreaseIntervalUsArg));
	if (lpccIncreaseFactorArg > 0.0)      Config::SetDefault("ns3::RdmaHw::LpccIncreaseFactor",        DoubleValue(lpccIncreaseFactorArg));
	if (lpccWrArg > 0.0)                  Config::SetDefault("ns3::RdmaHw::Lpcc_m_wr",                 DoubleValue(lpccWrArg));
	if (lpccKrArg > 0.0)                  Config::SetDefault("ns3::RdmaHw::Lpcc_m_kr",                 DoubleValue(lpccKrArg));
	if (lpccQueueTargetRatioArg > 0.0)    Config::SetDefault("ns3::RdmaHw::LpccQueueTargetRatio",      DoubleValue(lpccQueueTargetRatioArg));
	if (lpccDropCapHighArg > 0.0)         Config::SetDefault("ns3::RdmaHw::LpccDropCapHigh",           DoubleValue(lpccDropCapHighArg));
	if (lpccAiSuppressMultiplierArg > 0)  Config::SetDefault("ns3::RdmaHw::LpccAiSuppressMultiplier",  UintegerValue(lpccAiSuppressMultiplierArg));
	...
	while (!conf.eof())
	{
		...
		else if (key.compare("ERROR_RATE_PER_LINK") == 0)
		{
			...
			conf >> flowgen_start_time;
			std::cout << "FLOWGEN_START_TIME\t\t" << flowgen_start_time << '\n';
		} else if (key.compare("FLOWGEN_STOP_TIME") == 0) {
			conf >> flowgen_stop_time;
			std::cout << "FLOWGEN_STOP_TIME\t\t" << flowgen_stop_time << '\n';
	...
	if (!ParseTopologyHeader(node_num, switch_num, tors, link_num)) {
		NS_FATAL_ERROR("Invalid topology header in " << topology_file);
	}
	flowf >> flow_num;
	flowf.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			...
			rdmaHw->SetAttribute("LpccIncreaseFactor", DoubleValue(lpccBeta));
	...
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) {
			tcpSrcPortNumder[i] = rand_range(20000, 21000);
			tcpDstPortNumder[i] = rand_range(30000, 31000);
		}
	}
	...
	double stopTime = std::max(simulator_stop_time, flowgen_stop_time + 0.05);
	if (flow_num > 0) {
		InstallConfiguredFlows(stopTime);
	}
	Simulator::Stop(Seconds(stopTime));
	Simulator::Run();
	if (g_fct_output != nullptr) {
		fclose(g_fct_output);
	}
	Simulator::Destroy();
	NS_LOG_INFO("Done.");
	...
	endt = clock();
	std::cout << "SimulationSeconds " << (double)(endt - begint) / CLOCKS_PER_SEC << "\n";
}
