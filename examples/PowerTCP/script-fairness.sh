...
for algorithm in "${algs[@]}"; do
	...
	time ./build/examples/PowerTCP/ns3.39-powertcp-evaluation-fairness-debug --conf=$configFile --algorithm=${CCMODE[$algorithm]} --transportMode=${TRANSPORT[$algorithm]} --flowControlMode=${FLOWCTL[$algorithm]} --wien=$wien --delayWien=$delay --windowCheck=$window > "$RESULT_FILE" 2>&1 &
done
