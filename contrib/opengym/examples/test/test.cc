...
//     Time offset = Seconds(2.000000224);
//     for(const auto& flow : allFlows){
//         // cout << "flow.start_time: " << flow.start_time << "Simulator::Now() +  offset" << Simulator::Now() +  offset << endl;
//         if(flow.idx < flow_num && Seconds(flow.start_time) == (Simulator::Now() + offset)){
//             src = flow.src;
//             dst = flow.dst;
...
//         cout << "Node1 not found in allPaths.\n";
...
//         const vector<float> &allocation = pair.second;
//         if (flowIdx >= allFlows.size()) continue;
//         FlowInput flow = allFlows[flowIdx];
...
//         // 确保动作向量长度与候选路径数一致
//         // if (allocation.size() != paths.size()) continue;
        
//         double maxCompletionTime = 0.0;
//         double sumUtilization = 0.0;
        
//         // 对每条候选路径计算流完成时间和带宽利用率
//         for (size_t i = 0; i < paths.size(); i++)
//         {
//             const PathInfo &p = paths[i];
//             // 计算流完成时间：（分配比例 * 流大小）/ 链路带宽 + 传播时延 = 传输时延 + 传播时延
//             double path_time = (allocation[i] * flow.maxPacketCount) / p.bw + p.delay;
//             completionTimes.push_back(path_time);
//             maxCompletionTime = path_time > maxCompletionTime ? path_time : maxCompletionTime;
//             // 计算在该路径上实际分配的流量（假设 flow.maxPacketCount 表示流量大小）
//             double allocatedFlow = allocation[i] * flow.maxPacketCount;
//             // 带宽利用率：分配流量 / 路径带宽
//             double utilization = allocatedFlow / static_cast<double>(p.bw);
//             sumUtilization += utilization;
//         }
//         double avgUtilization = sumUtilization / paths.size();
...
//         // 奖励目标：最小化流完成时间 同时 最大化带宽利用率
//         double flowReward = -alpha * maxCompletionTime + beta * avgUtilization;
//         totalReward += flowReward;
//         flowCount++;
//     }
    
//     cout << "总奖励Reward" << totalReward <<endl;
//     // 返回所有流奖励的平均
//     if (flowCount > 0)
//         return totalReward / flowCount;
//     else
//         return 0.0;
// }
