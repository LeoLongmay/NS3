...
// app parameters 网络接口结构体：idx-索引；up-是否启用；delay-延迟；bw-带宽
struct Interface {
    uint32_t idx;
    /* data */
    bool up;
...
void conweave_history_print(){
    ...
    // 获取 ConWeave 刷新时间估计误差(Flush Time Estimation Error)
    if(0){
        assert(conweave_extraVOQFlushTime >= MicroSeconds(128) && "PARAMETER ERROR");   //条件
...
void CalculateRoute(Ptr<Node> host){
    ...
    for(int i = 0 ; i < (int)q.size(); i++){   //BFS遍历
        Ptr<Node> now = q[i];
        int d = dis[now];
        for(auto it = nbr2if[now].begin(); it != nbr2if[now].end(); it++){   //遍历邻居节点
            if(!it->second.up) continue;
            Ptr<Node> next = it->first;
            if(dis.find(next) == dis.end()){  //next 节点未被访问
                dis[next] = d + 1;
...
main (int argc, char *argv[]){
  ...
  uint32_t testArg = 0;
  std::string configFile = "default_config.txt";
  ...
  */
  ...
  for (uint32_t i = 0; i < node_num; i++) {
    if (n.Get(i)->GetNodeType() == 1) {  
      ...
      // uint32_t shift = 3;  // by default 1/8   修改代码 注释掉未使用的变量
