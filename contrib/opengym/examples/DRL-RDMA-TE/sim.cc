...
Time conga_flowletTimeout = MicroSeconds(100);   //100us 流片超时时间
...
Time letflow_agingTime = MilliSeconds(2);    //每隔一段时间清理未使用的映射条目

// Conweave parameters
Time conweave_extraReplyDeadline = MicroSeconds(4);       //额外回复超时期限
Time conweave_pathPauseTime = MicroSeconds(8);            //网络路径拥塞时发送数据包的暂停时间
Time conweave_txExpiryTime = MicroSeconds(100);           //等待CLEAR命令超时时间
Time conweave_extraVOQFlushTime = MicroSeconds(32);       //额外的排队时间
Time conweave_defaultVOQWaitingTime = MicroSeconds(500);  //默认的排队时间
bool conweave_pathAwareRerouting = true;                  //路径重路由感知


/*----------------simulation variables----------------*/
uint64_t one_hop_delay = 1000;   //单跳延迟 ns
uint32_t cc_model = 1;           //congestion control model parameters; 1：DCQCN
// 定义是否启用QCN（拥塞通知）协议、PFC（优先级流控制）协议，以及是否使用动态PFC阈值。
bool enable_qcn = true, enable_pfc = true, use_dynamic_pfc_threshold = true;
// 定义数据包负载大小、L2（数据链路层）分段大小和L2确认间隔。
uint32_t packet_payload_size = 1000, l2_chunk_size = 0, l2_ack_interval = 0;
double pause_time = 5;    //PFC 暂停帧时间，5us 流量控制过程中暂停的数据传输时间
//流量生成的开始时间和结束时间，单位为秒。流量生成在 [flowgen_start_time, flowgen_stop_time] 之间进行。
double flowgen_start_time = 1.0, flowgen_stop_time = 2.5, simulator_extra_time = 0.1; 
// queue length 监控时间，在这个仿真中没有使用
// uint32_t qlen_dump_interval = 100000000, qlen_mon_interval = 1000; //ns
uint64_t qlen_mon_start;       //队列长度监控开始时间
uint64_t qlen_mon_end;
uint32_t switch_mon_interval = 10000;   //交换机监控间隔时间
uint64_t cnp_mon_start;                 //CNP监控开始时间  CNP（Congestion Notification Protocol，拥塞通知协议）
uint64_t cnp_monitor_bucket = 100000;   //CNP监控时间间隔 
uint64_t irn_mon_start;                 //IRN监控开始时间  IRN（Incast Recovery Notification，拥塞恢复通知）
uint64_t irn_monitor_bucket = 100000;


// 定义文件指针，用于输出不同类型的仿真数据
FILE *pfc_file = NULL;
FILE *fct_output = NULL;
FILE *flow_input_stream = NULL;    //流输入流文件
FILE *cnp_output = NULL;           
FILE *est_error_output = NULL;     //估计错误输出文件
FILE *voq_output = NULL;            //VOQ（Virtual Output Queue，虚拟输出队列）输出文件
FILE *voq_detail_output = NULL;
FILE *uplink_output = NULL;
FILE *conn_output = NULL;           //连接输出文件
...
// CC models 拥塞控制模型
// 恢复间隔时间，重传时间在特定条件下出发重传，EWMA（Exponentially Weighted Moving Average，指数加权移动平均）增益，用于调整速率的平滑程度。
double alpha_resume_interval = 55, rp_timer = 300, ewma_gain = 1/16;
double rate_decrease_interval = 4;     //速率降低间隔时间
uint32_t fast_recovery_times = 1;      //快速恢复次数
// 速率调整参数名称，最小速率；dctcp_rate_ai：DCTCP速率
std::string rate_ai, rate_hai, min_rate = "100Mb/s";    
std::string dctcp_rate_ai = "1000Mb/s";

// 是否限制目标速率，是否允许L2链路重置为0，每条链路错误率
bool clamp_target_rate = false, l2_back_to_zero = false;
double error_rate_per_link = 0.0;
uint32_t has_win = 1;       //是否有窗口
uint32_t global_t = 1;      //全局时间
uint32_t mi_thresh = 5;     //最窗口小阈值
bool var_win = false, fast_react = true;    //是否有变化窗口，是否快速反应
bool multi_rate = true;    //是否启用多速率
bool sample_feedback = false;   //是否采样反馈
double u_target = 0.95;         //目标速率的比例因子
uint32_t int_multi = 1;         //整数倍速率的因子
bool rate_bound = true;         //是否速率限制，速率边界
unordered_map<uint64_t, uint32_t> rate2kmax, rate2kmin;   //速率到最大k值/最小k值的映射关系
unordered_map<uint64_t, double> rate2pmax;               //速率到最大P值的映射关系
// 定义一个映射，将节点索引（Id）映射到交换机节点指针（Ptr<SwitchNode>）
unordered_map<uint32_t, Ptr<SwitchNode>> idxNodeToR;     

// 配置链路故障场景、ACK优先级和缓冲区配置
uint64_t link_down_time = 0;                  //链路故障时间
...
map<Ptr<Node>,map<Ptr<Node>,vector<Ptr<Node>>>> nextHop;    //节点到下一跳的映射
map<Ptr<Node>,map<Ptr<Node>,uint64_t>> pairDelay;           //节点对之间的延迟
map<Ptr<Node>, map<Ptr<Node>, uint64_t>> pairTxDelay;       //节点对之间的传输延迟
...
std::vector<Ipv4Address> serverAddress;  // 服务器地址

// 读取拓扑 节点数量 链路数量
uint32_t node_num, switch_num, link_num;
std::vector<std::pair<uint32_t, uint32_t>> link_pairs;   //链路对:src, dst

// 流量生成器   每个主机生成流
std::unordered_map<uint32_t, uint32_t> flows_per_host;
uint32_t flow_id = 0;
std::unordered_map<uint32_t, uint16_t> portNumber;    //端口号
std::unordered_map<uint32_t, uint16_t> dportNumber;  //目的端口号
...
// 存储每对节点之间的路径计数
// map<Ptr<Node>, map<Ptr<Node>, int>> pathCount;

std::map<std::string, std::vector<float>> lastAction;
 ...
 * 通过链路信息直接获取任意两个节点之间的路径信息，存储每个定义
 * 3.流量信息：每条流量的源节点、目的节点、流量大小等信息
 */
Ptr<OpenGymSpace> MyGetObservationSpace(void){
    uint32_t flowNum = flow_num;  
    // 定义路径信息空间
    Ptr<OpenGymDictSpace> pathSpace = CreateObject<OpenGymDictSpace>();
    for (uint32_t i = 0; i < node_num - switch_num; ++i) {
        for (uint32_t j = 0; j < node_num - switch_num; ++j) {
            if (i != j) {
                ...
                if (outerIt != allPaths.end()) {      
                    ...
                    if (innerIt != outerIt->second.end()) {  
                        ...
                        const vector<PathInfo>& paths = innerIt->second;  
                        uint32_t maxPaths = paths.size();
                        cout << "源节点 " << i << " 目的节点" << j << " " << maxPaths << endl;
                        // for (const auto& path : paths) {
                        //     // 处理每条路径，例如输出路径信息
                        //     std::cout << "Path details: delay=" << path.delay 
                        //               << ", txDelay=" << path.txDelay 
                        //               << ", bw=" << path.bw 
                        //               << ", hops=" << path.hops << std::endl;
                        // }
                        for (uint32_t k = 0; k < maxPaths; ++k) {
                            std::vector<uint32_t> shape = {1};
                            Ptr<OpenGymBoxSpace> srcSpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> dstSpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> pathCountSpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> hopsSpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> delaySpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> txDelaySpace = CreateObject<OpenGymBoxSpace>();
                            Ptr<OpenGymBoxSpace> bwSpace = CreateObject<OpenGymBoxSpace>();
                            
                            std::string key = std::to_string(i) + "-" + std::to_string(j) + "-" + std::to_string(k + 1);
                            Ptr<OpenGymTupleSpace> pathInfoSpace = CreateObject<OpenGymTupleSpace>();
                            pathInfoSpace->Add(srcSpace);
                            pathInfoSpace->Add(dstSpace);
                            pathInfoSpace->Add(pathCountSpace);
                            pathInfoSpace->Add(hopsSpace);
                            pathInfoSpace->Add(delaySpace);
                            pathInfoSpace->Add(txDelaySpace);
                            pathInfoSpace->Add(bwSpace);
                            pathSpace->Add(key, pathInfoSpace);
                        }
                    } else {
                        // node1 到 node2 的路径信息不存在
                        std::cout << "No paths found from " << node1->GetId() << " to " << node2->GetId() << ".\n";
                    }
                } else {
                    // node1 在 allPaths 中不存在
                    std::cout << "Node1 not found in allPaths.\n";
                }
            }
        }
    }
    // 创建流量信息空间（源节点、目标节点、流量大小等）
    double currentTime = Simulator::Now().GetSeconds();  // 获取当前仿真时间
    Ptr<OpenGymDictSpace> flowSpace = CreateObject<OpenGymDictSpace>();
    for(uint32_t i = 0; i < flowNum; ++i) {
        if (flow_input.start_time <= currentTime + 2.1) {
            std::vector<uint32_t> shape = {1};
            Ptr<OpenGymBoxSpace> srcSpace = CreateObject<OpenGymBoxSpace>();
            Ptr<OpenGymBoxSpace> dstSpace = CreateObject<OpenGymBoxSpace>();
            Ptr<OpenGymBoxSpace> flowSizeSpace = CreateObject<OpenGymBoxSpace>();
            Ptr<OpenGymBoxSpace> startTimeSpace = CreateObject<OpenGymBoxSpace>();
            std::string key = "flow" + std::to_string(i);
            Ptr<OpenGymTupleSpace> flowInfoSpace = CreateObject<OpenGymTupleSpace>();
            flowInfoSpace->Add(srcSpace);
            flowInfoSpace->Add(dstSpace);
            flowInfoSpace->Add(flowSizeSpace);
            flowInfoSpace->Add(startTimeSpace);
            flowSpace->Add(key, flowInfoSpace);
        }
    }
    
    // 创建一个字典空间，包含链路信息和流量信息
    Ptr<OpenGymDictSpace> space = CreateObject<OpenGymDictSpace> ();
    space->Add("path", pathSpace);
    space->Add("flow", flowSpace);
...
// 整合所有流信息
void LoadFlows(const std::string& flowFilePath)
{
    std::ifstream flowFile(flowFilePath.c_str());
    if (!flowFile.is_open()) {
        std::cerr << "Error: Cannot open flow file " << flowFilePath << std::endl;
        return;
    }

    std::string flowLine;
    uint32_t flowIndex = 0;
    while (std::getline(flowFile, flowLine)) {
        std::istringstream iss(flowLine);
        FlowInput flow;
        // 根据实际的文件格式解析每个字段
        if (!(iss >> flow.src >> flow.dst >> flow.pg >> flow.maxPacketCount >> flow.start_time)) {
            std::cerr << "Error: Invalid flow file format in line: " << flowLine << std::endl;
            continue;
        }
        flow.idx = flowIndex++;
    ...
    flowFile.close();
    std::cout << "Loaded " << allFlows.size() << " flows." << std::endl;
}
/**
...
{
    Ptr<OpenGymDictSpace> actionSpace = CreateObject<OpenGymDictSpace>();
    double currentTime = Simulator::Now().GetSeconds(); 
    for(const auto &flow : allFlows){
        if(flow.start_time <= currentTime + 2.1){
            ...
            if (outerIt != allPaths.end()) {
                ...
                if (innerIt != outerIt->second.end()) {
                    uint32_t pathCount = innerIt->second.size();
                    
                    // 定义当前流的动作空间，其维度为该流对应的路径数量
                    std::vector<uint32_t> shape = {pathCount};
                    float low = 0.0;
                    float high = 1.0;
                    std::string dtype = TypeNameGet<float>();
                    Ptr<OpenGymBoxSpace> pathSpace = CreateObject<OpenGymBoxSpace>(low, high, shape, dtype);
                    
                    // 以流的唯一标识作为键，加入到动作空间字典中
                    std::string key = std::to_string(flow.src) + "-" + std::to_string(flow.dst) + "-" + std::to_string(flow.idx);
    ...
    // // cout << "allPaths[0]: " << allPaths.begin()->first->GetId() << endl;
    // // 遍历所有源节点和目的节点
    ...
    //         std::string key = std::to_string(srcNode->GetId()) + "-" + std::to_string(dstNode->GetId());
    //         actionSpace->Add(key, pathSpace);
...
{
    // 收集链路路径信息
    Ptr<OpenGymDictContainer> pathData = CreateObject<OpenGymDictContainer>();
    for (uint32_t i = 0; i < node_num - switch_num; ++i) {
        for (uint32_t j = 0; j < node_num - switch_num; ++j) {
            if (i != j) {
                Ptr<Node> node1 = n.Get(i);
                    ...
                    if (innerIt != outerIt->second.end()) {  
                        ...
                        for (const auto& path : paths) {
                            ...
                            std::cout << "Path details: delay=" << path.delay 
                                      ...
                                      << ", hops=" << path.hops << std::endl;
                            std::vector<uint32_t> shape = {1};
                            Ptr<OpenGymBoxContainer<uint32_t>> src = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint32_t>> dst = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint32_t>> pathCount = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint32_t>> hops = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint64_t>> delay = CreateObject<OpenGymBoxContainer<uint64_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint64_t>> txDelay = CreateObject<OpenGymBoxContainer<uint64_t>>(shape);
                            Ptr<OpenGymBoxContainer<uint64_t>> bw = CreateObject<OpenGymBoxContainer<uint64_t>>(shape);
                            src->AddValue(i);
                            dst->AddValue(j);
                            pathCount->AddValue(path.pathCount);
                            hops->AddValue(path.hops);
                            delay->AddValue(path.delay);
                            txDelay->AddValue(path.txDelay);
                            bw->AddValue(path.bw * 1000000);  //将带宽转换为bps

                            // 使用源节点、目的节点和路径编号作为键，将路径信息添加到字典中
                            std::string key = std::to_string(i) + "-" + std::to_string(j) + "-" + std::to_string(path.pathCount);
                            Ptr<OpenGymTupleContainer> pathInfo = CreateObject<OpenGymTupleContainer>();
                            pathInfo->Add(src);
                            pathInfo->Add(dst);
                            pathInfo->Add(pathCount);
                            pathInfo->Add(hops);
                            pathInfo->Add(delay);
                            pathInfo->Add(txDelay);
                            pathInfo->Add(bw);
                            pathData->Add(key, pathInfo);
                        }
                    } else {
                        // node1 到 node2 的路径信息不存在
                        // std::cout << "No paths found from " << node1->GetId() << " to " << node2->GetId() << ".\n";
    ...
    // 收集流量信息
    double currentTime = Simulator::Now().GetSeconds();  
    Ptr<OpenGymDictContainer> flowData = CreateObject<OpenGymDictContainer>();
    std::ifstream flowFile(flow_file.c_str());
    if (!flowFile.is_open()) {
    ...
    while (std::getline(flowFile, flowLine)) {
        ...
        double start_time;
        if (!(iss >> src >> dst >> pg >> maxPacketCount >> start_time)) {
            std::cerr << "Error: Invalid flow file format" << std::endl;
            continue;
        }
        // cout << "流信息" << endl;
        // cout << "src: " << src << " dst: " << dst << " pg: " << pg << " maxPacketCount: " << maxPacketCount << " start_time: " << start_time << endl;
        // cout << "currentTime: " << currentTime << endl;
        if(start_time <= currentTime + 2.1) {
            std::vector<uint32_t> shape = {1};
            Ptr<OpenGymBoxContainer<uint32_t>> srcContainer = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
            Ptr<OpenGymBoxContainer<uint32_t>> dstContainer = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
            // Ptr<OpenGymBoxContainer<uint32_t>> pgContainer = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
            Ptr<OpenGymBoxContainer<uint32_t>> flowSizeContainer = CreateObject<OpenGymBoxContainer<uint32_t>>(shape);
            Ptr<OpenGymBoxContainer<double>> startTimeContainer = CreateObject<OpenGymBoxContainer<double>>(shape);

            srcContainer->AddValue(src);
            dstContainer->AddValue(dst);
            // pgContainer->AddValue(pg);
            flowSizeContainer->AddValue(maxPacketCount);
            startTimeContainer->AddValue(start_time);

            // 使用流编号作为键，将流信息添加到字典中
            std::string key = "flow" + std::to_string(flowIndex);
            Ptr<OpenGymTupleContainer> flowInfo = CreateObject<OpenGymTupleContainer>();
            flowInfo->Add(srcContainer);
            flowInfo->Add(dstContainer);
            // flowInfo->Add(pgContainer);
            flowInfo->Add(flowSizeContainer);
            flowInfo->Add(startTimeContainer);
            flowData->Add(key, flowInfo);
    ...
    data->Add("flow", flowData);
    cout << "MyGetObservation收集状态成功"  << "data:\n" << data << endl;
    NS_LOG_UNCOND("MyGetObservation: " << data);
    return data;
}

// double ComputeRewardForFlow(const FlowInput& flow, const std::vector<float>& action) {
//     // 假设 flow 中包含流总量 flowsize
//     double F = static_cast<double>(flow.maxPacketCount);  // 或者其他定义的流量大小
//     // 从全局 allPaths 中获取该流对应的路径信息
//     Ptr<Node> srcNode = n.Get(flow.src);
//     Ptr<Node> dstNode = n.Get(flow.dst);
//     auto outerIt = allPaths.find(srcNode);
//     if (outerIt == allPaths.end()) return 0.0;
//     auto innerIt = outerIt->second.find(dstNode);
//     if (innerIt == outerIt->second.end()) return 0.0;
//     const vector<PathInfo>& paths = innerIt->second;
//     uint32_t pathCount = paths.size();

//     // 假设 action.size() == pathCount，对 action 归一化
...
//     std::vector<double> p;
//     for (float val : action) {
//         p.push_back(val / sumAction);  // 得到每条路径的比例
//     }

//     // 计算每条路径的完成时间和带宽利用率
//     std::vector<double> FCTs, utils;
//     for (uint32_t i = 0; i < pathCount; ++i) {
//         double a_i = p[i] * F;
//         double pathFCT = (a_i / F) * paths[i].txDelay + paths[i].delay;
//         FCTs.push_back(pathFCT);
//         double util = a_i / static_cast<double>(paths[i].bw);
//         utils.push_back(util);
//     }
//     // 总流完成时间取所有路径中的最大值
//     double maxFCT = *std::max_element(FCTs.begin(), FCTs.end());
//     // 平均带宽利用率
//     double avgUtil = std::accumulate(utils.begin(), utils.end(), 0.0) / pathCount;

//     // 奖励函数：使 FCT 最小、带宽利用率最大化
//     double beta = 1.0; // 可调参数    double reward = -maxFCT + beta * avgUtil;
//     return reward;
// }
...
float MyGetReward(void)
{
    ...
    cout << "allFlows.size(): " << allFlows.size() << endl;
    cout << "MyGetReward创建成功" << "MyGetReward: finalReward = " << endl;
    return 0.0;
    // for (const auto &flow : allFlows) {
    //     if (flow.start_time <= currentTime + 2.1) {
    //         // 构造动作空间中对应该流的键（此处以 "src-dst" 作为键）
    //         std::string key = std::to_string(flow.src) + "-" + std::to_string(flow.dst) + "-" + std::to_string(flow.idx);

    //         // 如果未找到对应的动作，则跳过该流
    //         cout << "lastAction.size(): " << lastAction.size() << " lastAction: " << endl;
    //         if (lastAction.find(key) == lastAction.end()) {
    //             continue;
    //         }
    //         std::vector<float> action = lastAction[key];

    //         // 根据流的 src 和 dst，从全局 allPaths 中查找候选路径
    //         Ptr<Node> srcNode = n.Get(flow.src);
    //         Ptr<Node> dstNode = n.Get(flow.dst);
    //         auto outerIt = allPaths.find(srcNode);
    //         if (outerIt == allPaths.end()) continue;
    //         auto innerIt = outerIt->second.find(dstNode);
    //         if (innerIt == outerIt->second.end()) continue;
    //         const std::vector<PathInfo>& paths = innerIt->second;
    //         uint32_t pathCount = paths.size();

    //         // 若动作向量的维度与候选路径数不匹配，则跳过该流
    //         if (action.size() != pathCount) continue;

    //         // 对动作向量归一化，得到各路径分配比例 p_i
    //         double sumAction = 0.0;
    //         for (float val : action) {
    //             sumAction += val;
    //         }
    //         if (sumAction == 0) continue;  // 避免除以 0
    //         std::vector<double> proportions;
    //         for (float val : action) {
    //             proportions.push_back(val / sumAction);
    //         }

    //         // 计算每条路径的流完成时间和带宽利用率
    //         std::vector<double> FCTs;
    //         std::vector<double> utils;
    //         // 这里使用 flow.maxPacketCount 作为流大小（根据实际情况可调整为流量大小的单位）
    //         double F = static_cast<double>(flow.maxPacketCount);
    //         for (uint32_t i = 0; i < pathCount; ++i) {
    //             // 流完成时间计算：比例 * 传输时延 + 传播时延
    //             double txDelay = static_cast<double>(paths[i].txDelay);
    //             double delay = static_cast<double>(paths[i].delay);
    //             double pathFCT = proportions[i] * txDelay + delay;
    //             FCTs.push_back(pathFCT);

    //             // 带宽利用率：分配流量 / 路径带宽
    //             double allocatedFlow = proportions[i] * F;
    //             double util = allocatedFlow / static_cast<double>(paths[i].bw);
    //             utils.push_back(util);
    //         }
    //         // 对该流，流完成时间取所有路径中的最大值
    //         double maxFCT = *std::max_element(FCTs.begin(), FCTs.end());
    ...
    //         // 定义权重参数 beta，用于平衡 FCT 与带宽利用率
    //         double beta = 1.0;
    //         double reward = -maxFCT + beta * avgUtil;

    //         totalReward += reward;
    //         activeFlowCount++;
    //     }
    // }
    // // 如果没有激活流，则奖励设为 0，否则取所有激活流奖励的平均值
    // double finalReward = (activeFlowCount > 0) ? totalReward / activeFlowCount : 0.0;
    // NS_LOG_UNCOND("MyGetReward: finalReward = " << finalReward);
    // return static_cast<float>(finalReward);
...
{
    cout << "MyExecuteActions函数执行" << endl;
    NS_LOG_UNCOND ("MyExecuteActions: " << action);

    Ptr<OpenGymBoxContainer<uint32_t> > box = DynamicCast<OpenGymBoxContainer<uint32_t> >(action);
    cout << "box: " << box << endl;
    std::vector<uint32_t> actionVector = box->GetData();
    cout << "actionVector.size(): " << actionVector.size() << endl;
    for (uint32_t i = 0; i < actionVector.size(); ++i) {
        cout << "actionVector[" << i << "]: " << actionVector[i] << endl;
    }
...
void ReadFlowInput(){
    if (flow_input.idx < flow_num){
        ...
        // std::cout << "flow_input.src GetNodeType" << n.Get(flow_input.src)->GetNodeType() << std::endl;
        // std::cout << "flow_input.dst GetNodeType" << n.Get(flow_input.dst)->GetNodeType() << std::endl;
...
void ScheduleFlowInputs(FILE *infile){
    ...
    while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
        // std::cout << "进入循环：flow_input.idx: " << flow_input.idx << std::endl;
...
void cnp_freq_mointoring(FILE *fout, Ptr<RdmaHw> rdmahw){
    // std::cout << "CNP 写入文件调用" << rdmahw->cnp_total << std::endl;
 ...
 * 在RNIC网卡上的 活动连接数量
 */
void periodic_monitoring(FILE *fout_voq, FILE *fout_voq_detail, FILE *fout_uplink, FILE *fout_conn, uint32_t *lb_model){
    uint32_t lb_model_val = *lb_model;
    ...
    for(const auto &tor2If : torId2UplinkIf){
        ...
        if(lb_model_val == 9){   // 9:conweave 监控VOQ
    ...
    if(Simulator::Now() < Seconds(flowgen_stop_time + 0.05)){   
        // 回调函数
        Simulator::Schedule(NanoSeconds(switch_mon_interval),&periodic_monitoring, fout_voq, fout_voq_detail, fout_uplink,fout_conn, lb_model);
    }
...
void conga_history_print(){
    ...
    std::cout << "Number of flowlet's timeout:" << LetflowRouting::nFlowletTimeout
              << "\n Conga's timeout:   " << letflow_flowletTimeout << std::endl;
}
...
void conweave_history_print(){
    ...
    std::cout << "parameter - extraReplyDeadline:" << conweave_extraReplyDeadline << std::endl;  //额外回复截止时间
    std::cout << "parameter - extraVOQFlushTime:" << conweave_extraVOQFlushTime << std::endl;    //额外VOQ刷新时间
    std::cout << "parameter - txExpiryTime:" << conweave_txExpiryTime<< std::endl;               //输出发送超时
    std::cout << "parameter - defaultVOQWaitingTime:" << conweave_defaultVOQWaitingTime << std::endl;   //默认VOQ等待时间
    std::cout << "parameter - pathPauseTime:" << conweave_pathPauseTime << std::endl;                   //路径暂停时间
    std::cout << "parameter - pathAwareRerouting:" << conweave_pathAwareRerouting << std::endl;         //路径感知重路由
...
void FindAllPaths(Ptr<Node> host, Ptr<Node> target, vector<Ptr<Node>>& path, uint64_t totalDelay, uint64_t totalTxDelay, uint64_t minBw, uint32_t& count, uint32_t src) {
    ...
    for (auto it = nbr2if[host].begin(); it != nbr2if[host].end(); it++) {
        ...
        uint64_t newBw = std::min(minBw, edgeBw);
        // 只有当邻居节点为交换机节点 才进行递归
        ...
        if (next == target) {// 如果到达目标节点，则记录路径和相关信息
            ...
            if(path_output == NULL){
                std::cerr << "Error: Cannot open file " << path_output_file << std::endl;
                exit(1);
            }
...
void CalculateAllPaths(NodeContainer &n) {
    ...
    Ptr<Node> node3 = n.Get(2);
    Ptr<Node> node4 = n.Get(32);
...
int main(int argc, char *argv[]){
    ...
    uint32_t testArg = 0;
    std::string configFile = "~/ns-allinone-3.29/ns-3.29/mix/output/664264916/config.txt";
    ...
    std::cout << "Reading configuration file...读取文件" << std::endl;
    ...
    if (key.compare("FLOW_INPUT_FILE") == 0) {
        ...
        conf >> v;
        path_output_file = v;
        std::cerr << "PATH_OUTPUT_FILE\t\t\t" << path_output_file << "\n";
        ...
        std::cout << "写入 FLOWGEN_START_TIME" << std::endl;
        double v;
        conf >> v;
    ...
    flowf >> flow_num;
    std::cout << "读取文件拓扑和流量" << "Node num: " << node_num << ", Switch num: " << switch_num << ", Link num: " << link_num
              << ", Flow num: " << flow_num << std::endl;
    ...
    for(uint32_t i = 0; i < switch_num; i++){
        ...
        topof >> sid;
        node_type[sid] = 1;
        // std::cout << "创建节点" << node_type[sid] << sid << std::endl;
    }
    for(uint32_t i = 0; i < node_num; i++){
        if(node_type[i] == 0){
            n.Add(CreateObject<Node>());
            // std::cout << "创建节点------" << i << n.Get(i) << std::endl;
        }else{
            Ptr<SwitchNode> sw = CreateObject<SwitchNode>();
            n.Add(sw);
            sw->SetAttribute("EcnEnabled", BooleanValue(enable_qcn));
            // std::cout << "交换机节点-------" << i << n.Get(i) << std::endl;   
        }
    }
    ...
    for(uint32_t i = 0; i < node_num; i++){
        if(n.Get(i)->GetNodeType() == 0){
            ...
            // std::cout << "分配IP" << serverAddress[i] << std::endl;
        }
    }
    ...
    uv->SetStream(50);
    // std::cout << "创建链路错误模型-------" << rem << uv << std::endl;
    ...
    for(uint32_t i = 0; i < link_num; i++){
        ...
        // std::cout << "创建链路----------" << snode << dnode << std::endl;
        // 设置链路属性
        ...
        // NetDeviceContainer d = qbb.Install(snode, dnode);
        // std::cout << "拓扑建立链路对" << DynamicCast<QbbNetDevice>(d.Get(0)) << DynamicCast<QbbNetDevice>(d.Get(1)) << std::endl;  
    ...
    if (cc_model == 1) {
        std::cout << "模式为 1 CNP output file: " << cnp_output_file << std::endl;
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        if (n.Get(i)->GetNodeType() == 0) {  // is server
            ...
            // std::cout << "Create RdmaHw for node " << i << std::endl;
            ...
            if (cc_model == 1) {
                // std::cout << "调度事件，时间: " << cnp_mon_start << "ns" << std::endl;
                // std::cout << "当前仿真时间: " << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;
                // 问题
                Simulator::Schedule(NanoSeconds(cnp_mon_start), &cnp_freq_mointoring, cnp_output,
                                    rdmaHw);
                // Simulator::Schedule(NanoSeconds(0), &test);
                // std::cout << "事件调度已完成。" << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;
            }
            ...
            rdma->Init();
            // std::cout << "RdmaHw for node----- " << rdma << node << rdmaHw << std::endl;
    ...
    printf("lb_model = %d\n", lb_model);
    if (lb_model == 3 || lb_model == 6 || lb_model == 9) {  
        NS_LOG_INFO("Configuring Load Balancer's Switches");
        for (auto &pair : link_pairs) {
            Ptr<Node> probably_host = n.Get(pair.first);
            Ptr<Node> probably_switch = n.Get(pair.second);
            // std::cout << "Configuring Load Balancer's Switches-----" << probably_host << probably_switch << std::endl;
            // 主机——交换机 链接
            if (probably_host->GetNodeType() == 0 && probably_switch->GetNodeType() == 1) {
                Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(probably_switch);
                uint32_t hostIP = serverAddress[pair.first].Get();
        ...
        for (auto i = nextHop.begin(); i != nextHop.end(); i++) {  
            if (i->first->GetNodeType() == 1) {                    
                ...
                // std::cout << "Configuring Load Balancer's Switches-----" << nodeSrc << swSrc << swSrcId << std::endl;
                if (swSrc->m_isToR) {
                    // printf("--- ToR Switch %d\n", swSrcId);
    ...
    MyGetActionSpace();
    MyGetReward();
    MyExecuteActions(CreateObject<OpenGymBoxContainer<uint32_t>>());
    ...
    Simulator::Schedule(Seconds(flowgen_start_time), &periodic_monitoring, voq_output, voq_detail_output,
                        uplink_output, conn_output, &lb_model);
    ...
    std::cout << "当前仿真Run时间: " << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;
    
    Simulator::Run();
    std::cout << "当前仿真Run时间: " << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;

    std::cout << "当前仿真Destroy前时间: " << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;
    // openGymInterface->NotifySimulationEnd();
    Simulator::Destroy();
    std::cout << "当前仿真Destroy后时间: " << Simulator::Now().GetNanoSeconds() << "ns" << std::endl;
    std::cout << "Total number of packets:  " << RdmaHw::nAllPkts << std::endl;
    std::cout << "\n Done" <<  std::endl;
